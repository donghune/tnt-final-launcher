...
// 런처별로 갈라야 한다. 호백/훈민 런처도 MC 26.1.2 를 쓰는데 모드 구성이 달라서,
// 같은 폴더를 공유하면 서로의 mods 폴더를 덮어써 게임이 안 뜬다.
const dataPath = path.join(sysRoot, '.tntfinallauncher')
...
exports.getTempNativeFolder = function(){
    // 런처마다 달라야 한다. Helios 런처는 시작할 때 자기 natives 폴더를 통째로 지우는데,
    // 이름이 같으면 훈민/호백 런처와 %TEMP% 에서 충돌해 서로의 게임 부팅을 깨뜨린다.
    return 'TntNatives'
}
