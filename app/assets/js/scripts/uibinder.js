...
// Guard against a duplicate delivery: the main process replays the result for us
// (see the distributionIndexRequest handshake below), so the event can arrive twice.
let distributionIndexHandled = false
ipcRenderer.on('distributionIndexDone', async (event, res) => {
    if(distributionIndexHandled){
        return
    }
    distributionIndexHandled = true
    if(res) {
        const data = await DistroAPI.getDistribution()
        syncModConfigurations(data)
        ensureJavaSettings(data)
...
// The preloader may have finished before this listener existed (a local distro
// server answers in milliseconds). Ask the main process to replay the result.
ipcRenderer.send('distributionIndexRequest')
